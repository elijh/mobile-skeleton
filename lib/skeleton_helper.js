...
function buildListItem(values) {
    if (values.count) {
      return "<li><a data-id='{id}'>{label}<span class='ui-li-count ui-btn-up-c ui-btn-corner-all'>{count}</span></a></li>".interpolate(values);
    } else {
      return "<li><a data-id='{id}'>{label}</a></li>".interpolate(values);
    }
}
